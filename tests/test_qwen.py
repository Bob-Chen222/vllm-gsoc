...
def test_qwen2_regular_run(model: str, model_impl: str):
    ...
    input_ids = jax.random.randint(key, shape=(batch, seq_len), minval=0, maxval=32)
    ...
    print("vllm_hidden:", vllm_hidden.shape)

    # 7. Compare.
    # assert torch.allclose(vllm_hidden, hf_hidden, atol=1e-4, rtol=1e-4) 

if __name__ == "__main__":
    test_qwen2_regular_run("Qwen/Qwen2-1.5B-Instruct", "auto")