...
class UnquantizedEmbeddingMethod(QuantizeMethodBase):
    ...
    def apply(self,
              layer: flax.nnx.Module,
              x: jax.Array,
              bias: Optional[jax.Array] = None) -> jax.Array:
        return dispatch_unquantized_gemm_tpu()(x, layer.weight, bias)

    def embedding(self, layer: flax.nnx.Module,
                  input_: jax.Array) -> jax.Array:
        return jnp.take(layer, input_, axis=0)
...
class VocabParallelEmbedding(flax.nnx.Module):
    ...
    def __init__(self,
                 num_embeddings: int,
                 embedding_dim: int,
                 params_dtype: Optional[jax.dtype] = None,
